...
public class Water : MonoBehaviour
{
    ...
    private GameObjectPool SplashPool;
    // Layer Mask for raycast ignoring
    ...
    void Awake()
    {
        ...
        {
            SplashPool = new GameObjectPool(3, SplashPrefab);
        }
    }
    ...
    void InstantiateSplashPrefab(Collider col)
    {
        // Some objects may not need a splash at all, don't do anything in this case
        if (SplashPool != null)
        {
            // Stop Camera from causing a splash
            if (col.tag != "MainCamera")
            {
                // Get Splash prefab ffrom our pool
                GameObject SplashGO = SplashPool.New();

                // Raycast for the water, using the layermask
                RaycastHit hit;
                if (Physics.Raycast((col.transform.position + new Vector3(0, 2f, 0)), Vector3.down, out hit, 10f, layerMask))
                {
                    SplashGO.gameObject.transform.position = hit.point;
                }
